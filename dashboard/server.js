...
// Broadcast to all clients. This is what pushes data from server side to client dashboard
...
function processEvent(topic, message) {
  ...
  // Create event object with before/after CDC info
  const event = {
...
async function startKafkaConsumer() {
  ...
  try 
  {
    ...
    // Run consumer. long-running loop that polls apache kafka broker for messages from topics
    await consumer.run({
      ...
      eachMessage: async ({ topic, partition, message }) => {
        try 
        {
          ...
          processEvent(topic, value);  // business logic. process debezium data sent to kafka topic
        } 
