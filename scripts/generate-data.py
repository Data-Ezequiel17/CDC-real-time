...
def create_customer(conn):
    ...
    """, (
        fake.unique.email(), fake.first_name(), fake.last_name(), fake.phone_number(), tier, round(random.uniform(0, 5000), 2)
    ))
...
def update_customer(conn):
    ...
    if random.random() > 0.5:
        ...
        cursor.execute("""
            UPDATE ecommerce.customers SET tier = %s WHERE id = %s
        """, (new_tier, customer[0]))
    else:
        additional_spent = round(random.uniform(50, 500), 2)
        cursor.execute("""
            UPDATE ecommerce.customers SET total_spent = total_spent + %s WHERE id = %s
...
def run_demo(interval=2, duration=None):
    ...
    """func names and the weights. 
        weights are used to make the functions 'x' times more likely to be selected in the random.choices() below"""
    operations = [
        (create_customer, 10),
        (update_customer, 15),
        (create_order, 25),
    ...
    while running:
        ...
        #selects a random update or create function from above to execute
        try: 
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--interval', type=float, default=2, help='Interval between operations in seconds (default: 2)')
    parser.add_argument('-d', '--duration', type=int, default=None, help='Duration to run in seconds (default: unlimited)')
